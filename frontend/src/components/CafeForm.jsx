...
const CafeForm = () => {
  ...
  const fetchCafeDetails = (id) => {
    axios.get(`${import.meta.env.VITE_APP_BACKEND_URL}:5001/cafe/${id}`)
      .then(response => {
  ...
  const handleUpdateCafe = (formData) => {
    axios.put(`${import.meta.env.VITE_APP_BACKEND_URL}:5001/cafe`, formData, {
  ...
  const handleAddCafe = (formData) => {
    axios.post(`${import.meta.env.VITE_APP_BACKEND_URL}:5001/cafe`, formData, {
      headers: { 'Content-Type': 'multipart/form-data' }  // Ensure correct content type
    })
    .then(() => {
